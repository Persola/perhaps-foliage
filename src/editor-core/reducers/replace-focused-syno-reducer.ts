...
export default ((state: StateSelector, action: ReplaceFocusedSyno, draftState: MutableEditorState, integration: LanguageIntegration): void => {
  ...
  if (focusSyno.parent) {
    ...
    const typesAllowedUnderParent: string[] = Object.values(grammar[parent.syntype].children) // $FlowIssue: poorly typed ECMA built-in (map)
  ...
  synoMapReducer(state, action, draftSynoMap, draftState, newSynoAttrs, newSynoId);
  draftState.focus = {
    synoId: newSynoId,
    presnoIndex: null,
    charIndex: null
