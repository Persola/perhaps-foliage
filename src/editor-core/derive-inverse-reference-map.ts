...
const createOrAdd = (irm: MutableInverseReferenceMap, referantId: SynoId, referrerId: SynoId) => {
  irm[referantId] = irm[referantId] || new Set();
  irm[referantId].add(referrerId);
};

const depthFirstBuildInverseReferences = (irm: MutableInverseReferenceMap, synoMap: SynoMap, currentId: SynoId) => {
