...
const nor = (firstArg: BooleanLiteral | BooleanLiteralAttrs, secondArg: BooleanLiteral | BooleanLiteralAttrs): BooleanLiteral => {
...
export default ((argumentz: BooleanLiteral[]): InterpretationResolution => {
  ...
  if (!isBoolean(argumentz[0]) || !isBoolean(argumentz[1])) {
    ...
    return {
      success: false,
      error: {
        message: `NOR recieved non-boolean argument '${String(badArg)}' (${typeof badArg})`
      }
    };
  }
