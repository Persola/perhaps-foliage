...
export default ((interpreter: (...args: Array<any>) => any, parentScope: [], argumentz: Argument[], state: StateSelector): [Argument, BooleanLiteral][] => {
  ...
  argumentz.forEach((arg: Argument) => {
    ...
    const argResolution = interpreter(state.getSyno(arg.value.id), parentScope, state);
